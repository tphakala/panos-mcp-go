...
These authentication and log-forwarding server profiles are device-scoped: on a firewall they resolve to a `vsys` (or `shared`, for the three authentication server profiles and syslog); on Panorama a `template`, `template_stack`, `shared`, or `panorama` selection is required, and a `template` or `template_stack` can be narrowed to a specific `template_vsys`. `panorama` is the management-plane scope, which is where Panorama's own appliance-level configuration lives, as opposed to a template pushed to managed firewalls. The firewall `vsys` field is firewall-only: on Panorama it is rejected rather than silently retargeted, so use `template_vsys` to narrow within a template. The families with no shared scope are snmp-trap, email and authentication profiles (the authentication profile is documented below); syslog, the third log-forwarding profile, does have one. Secrets (bind and shared-secret passwords, SNMP communities and v3 passwords, SMTP passwords) are write-only: they are accepted on create and update but never returned, and a get reports only a `has_<secret>` boolean.
